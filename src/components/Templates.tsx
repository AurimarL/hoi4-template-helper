...
export function TemplateDesigner(
    { supportGrid, combatGrid, isSlotLocked, handleCellClick }:
        {
            combatGrid: (Unit | null)[],
            supportGrid: (Unit | null)[],
            isSlotLocked: (grid: "combat" | "support", index: number, selectedUnit?: Unit | null) => boolean,
    ...
    return (
        <div className="flex-none p-2 m-2 rounded-lg shadow-lg border border-green-700">
            <h1 className="text-3xl font-bold mb-6 text-green-400">Battalion Template Designer</h1>
                        ...
                        {combatGrid.map((unit, index) => {
                            ...
                            return (
                                <div className='flex justify-center' key={index}>
                                    ...
                                    <Button
                                        variant="outline"
                                        className={`w-12 h-12 md:w-14 md:h-14 text-2xl ${isLocked
                                        ...
                                        disabled={isLocked}
                                    >
                                        {unit ? unit.icon : isLocked ? <Lock className="w-6 h-6 text-gray-400" /> : <Plus className="w-6 h-6 text-green-400" />}
                                    </Button>
                                </div>
...
export function TemplateStats({ totalStats }: { totalStats: UnitStats }) {
    return (
        <Card className="flex-none p-2 m-2 bg-gray-700 border-green-700 shadow-lg">
            <CardHeader>
                <CardTitle className="text-2xl text-green-400">Template Stats</CardTitle>
            </CardHeader>
            <CardContent>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div>
                        <h3 className="text-base font-semibold mb-2 text-green-400">Base Stats</h3>
                        <table className="w-full">
                            <tbody>
                                {[
                                    ...
                                    <tr key={label} className="border-b border-gray-600 last:border-b-0">
                                        <td className="py-1">{label}</td>
                                        <td className="py-1 text-right">
                                            {key ? (totalStats[key as keyof UnitStats]?.toFixed(2) || '0.00') : '0.00'}
                                            {unit}
                                        </td>
                                    </tr>
                                ))}
                    ...
                    <div>
                        <h3 className="text-base font-semibold mb-2 text-green-400">Combat Stats</h3>
                        <table className="w-full">
                            <tbody>
                                {[
                    ...
                    <div>
                        <h3 className="text-base font-semibold mb-2 text-green-400">Equipment</h3>
                        <table className="w-full">
                            <tbody>
                                {[
                                    ['👥 Manpower', 'manpower'],
                                    ...
                                    <tr key={label} className="border-b border-gray-600 last:border-b-0">
                                        <td className="py-1">{label}</td>
                                        <td className="py-1 text-right">
                                            {key ? (totalStats[key as keyof UnitStats]?.toFixed(0) || '0') : '0'}
                                            {unit}
                                        </td>
                                    </tr>
                                ))}
