...
import { Unit, UnitCategory, units, UnitStats } from "@/lib/units";
import { TemplateDesigner, TemplateStats } from "@/components/Templates";
import { CategoryDialog, UnitDialog } from "@/components/Dialog";

export default function Page() {
  const [combatGrid, setCombatGrid] = useState<(Unit | null)[]>(
    Array(25).fill(null)
  );
  const [supportGrid, setSupportGrid] = useState<(Unit | null)[]>(
    Array(5).fill(null)
  );
  ...
  const handleCategorySelect = (
    category: "infantry" | "mobile" | "armored"
  ) => {
  ...
  const handleUnitSelected = (name: string) => {
    ...
    const selectedUnit = units.find(
      (unit) => unit.category === selectedCategory && unit.name === name
    );
  ...
  const handleUnitSelect = (unit: Unit) => {
    if (selectedCell) {
      if (selectedCell.grid === "support" && unit.category === "support") {
        ...
        selectedCell.grid === "combat" &&
        !isSlotLocked("combat", selectedCell.index, unit)
      ) {
  ...
  const isSlotLocked = (
    ...
    if (grid === "support") {
      ...
      if (selectedUnit && unitAbove.category !== selectedUnit.category)
        return true; // Slot is locked if the unit above is of a different type
      return false;
    }
  };
  ...
  useEffect(() => {
    const calculateTotalStats = () => {
      ...
      const allUnits = [...supportGrid, ...combatGrid].filter(
        (unit) => unit !== null
      ) as Unit[];

      for (const stat in newTotalStats) {
        if (stat === "org" || stat === "hardness") {
          newTotalStats[stat as keyof UnitStats] =
            allUnits.reduce(
              (sum, unit) => sum + unit.stats[stat as keyof UnitStats]!,
              0
            ) / allUnits.length;
        } else if (stat === "speed") {
          const combatUnits = allUnits.filter(
            (unit) => unit.category !== "support"
          );
          newTotalStats[stat as keyof UnitStats] =
            combatUnits.length > 0
              ? Math.min(...combatUnits.map((unit) => unit.stats.speed))
              : 0;
        } else if (stat === "armor") {
          const highestArmor = Math.max(
            ...allUnits.map((unit) => unit.stats.armor || 0)
          );
          const averageArmor =
            allUnits.reduce((sum, unit) => sum + (unit.stats.armor || 0), 0) /
            allUnits.length;
          newTotalStats[stat as keyof UnitStats] =
            0.4 * highestArmor + 0.6 * averageArmor;
        } else if (stat === "piercing") {
          const highestPiercing = Math.max(
            ...allUnits.map((unit) => unit.stats.piercing || 0)
          );
          const averagePiercing =
            allUnits.reduce(
              (sum, unit) => sum + (unit.stats.piercing || 0),
              0
            ) / allUnits.length;
          newTotalStats[stat as keyof UnitStats] =
            0.4 * highestPiercing + 0.6 * averagePiercing;
        } else {
          newTotalStats[stat as keyof UnitStats] = allUnits.reduce(
            (sum, unit) => sum + (unit.stats[stat as keyof UnitStats] || 0),
            0
          );
        }
      }
    ...
    <main className="">
      <div className="my-8 text-center">
        <h1 className="text-4xl font-bold text-green-400 mb-2">
          HOI4 Land Template Designer
        </h1>
        <p className="text-lg">
          Create and optimize your Hearts of Iron IV division templates with
      ...
      <div className="mt-6 flex justify-around items-center">
        <Button
          variant="destructive"
          onClick={handleReset}
          className="flex space-y-2 items-center bg-red-700 hover:bg-red-600"
        >
